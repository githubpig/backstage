...
(function($) {
	...
	function dateMessage(rid, treeLineCount, rootData, childrenData, opts) {
		...
		if (childrenData != undefined) {
			if (rid != "")rid = 'pid="' + rid + '"';
			var rootHTML = '<li id="tm-tree-' + rootData.opid + '" opid="' + rootData.opid + '" ' + rid + 'lineNum="' + treeLineCount + '"'+'>';
			if (isEmpty(opts.type)) {
				rootHTML += '<div class="">'
						...
						+ rootData.name + '</a></label>'+opts.outhtml+'</div>';
			} else {
				rootHTML += '<div class="">'
						+ getTreeLine(treeLineCount)
			...
			rootHTML += '<ul id="ul_'+rootData.opid +'" style="display:none;">';
			treeLineCount++;
			if(childrenData!=undefined){
	...
	function zyw_remove(obj){
		obj.closest("li").remove();
	}
	/*初始化事件*/
	function initEvent($this, opts) {
		var count = opts.count;//后台的总条数
		console.log(count);
		$this.find(".remove").on("click",function(){
			var opid = $(this).siblings("label").find("a").attr("opid");
			//alert(opid);
			if(opts.callback){opts.callback.call(this,opid)};
		});
		
		$this.find(".zyw_add").on("click",function(){
			var opid = $(this).siblings("label").find("a").attr("opid");
			var $pid = $("#tm-tree-"+opid);//当前点击的目录id
			var lineNum = $pid.attr("lineNum")*1+1;
			//alert(lineNum);
			//alert($pid);
			//var li_num = $pid.find("#ul_"+opid).find("li").length+1;
			var this_id = ++count;
			var addhtml  = '<li id="tm-tree-'+this_id+'" opid="'+this_id+'" pid="'+opid+'"> '
							+'<div class=""> '
							+ getTreeLine(lineNum)
							+'	<div class="node"></div> '
							+'	<div class="file"></div> '
							+'	<div class="file_checkbox"><span class="tm-tree-checkbox" data-title="删除" opid="'+this_id+'" pid="'+opid+'"></span></div> '
							+'	<label class="tm-ui-tree-name" id="tm-tree-name-'+this_id+'"><a href="javascript:void(0);" opid="'+this_id+'" pid="'+opid+'">新建---</a></label> '
							+ opts.outhtml
							+'</div>'
							+'</li>';
			$pid.find("#ul_"+opid).append(addhtml);
		});
				...
				if (!$(this).hasClass("tm-tree-"+opts.type+"-checked")) {
					...
					$(this).parents("li").each(function() {
						var $p = $(this);
							...
							var clen = $p.find(".tm-tree-"+opts.type+"-checked").length;
							if (clen != 0) {
								$p.find(".tm-tree-"+opts.type).first().removeClass("tm-tree-"+opts.type+"-checked").removeClass("tm-tree-"+opts.type+"-focus");
							} else {
	...
	$.fn.tmTree.defaults = {
		...
		rightMenu : false,
		expand : true,//是否显示展开和收起
		expandCount : 0,//展示第几个节点
